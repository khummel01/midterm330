...
function findMovie() {
  ...
  fetch(`https://api.themoviedb.org/3/search/movie?api_key=270ef537760087ddcea25e06616b754d&query=${title}`, config) //return a promise that contains details about the response
    .then(function(response) {
      return response.json();
    })
    .then(function(data) {
      let x = data[0][title]; //change to TMDB specific
      console.log(data)
